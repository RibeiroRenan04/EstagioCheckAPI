...
| `database/002` … `009` | Evoluções posteriores. | **Manual**: executadas à mão, na ordem. Não são migrations do EF. |
...
O script executa `migration.sql` e depois `002` … `009`, parando no primeiro erro.
...
`005` foi escrita para poder rodar mais de uma vez. As demais **não** são
